...
def start_video_server(ip_addr):
  proc = subprocess.Popen(
      "python server/video_server.py --host=%s --port=%d > logs/video_server.log 2>&1"
      % (ip_addr, VID_SERVER_PORT),
      stdout=sys.stdout,
      stderr=sys.stderr,
      shell=True)
...
def start_mm_cmd(trace, ip_addr):
  print 'Running...'
  proc = subprocess.Popen(mm_cmd(trace, ip_addr),
                          stdout=sys.stdout,
                          stderr=sys.stderr,
                          shell=True)
  return proc
...
def parse_abr_log():
  ...
  if nchunks == 0:
    ...
    return 0
  avg_qoe /= nchunks
  print '%d chunks fetched, total rebuffer time = %fs, Avg QoE = %f' % (
      nchunks, rebuf, avg_qoe)
  return {
      'num_chunks': nchunks,
      'rebuf': rebuf,
      'score': avg_qoe,
...
def setup_virtual_ip(ip_addr):
  ...
  if ret != 0:
    ...
    ifname = subprocess.check_output(
        "ifconfig -a | sed 's/[ \t].*//;/^\(lo\|\)$/d' | head -n 1",
        shell=True)
...
def cleanup():
  ...
  pid = os.getpid()
  os.system(
      "ps | grep python | grep -v '%d' | cut -d ' ' -f 1,2 | xargs kill" % pid)

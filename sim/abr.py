...
class ABRSimulator():
  ...
  def __init__(self, trace, start_t_idx, video_file, seed, startup_penalty,
               rebuffer_penalty, smooth_penalty):
    # set up seed
    self.set_seed(seed)
    self.trace, self.curr_t_idx = trace, start_t_idx
    self.video = vid = video.Video(video_file)
    ...
    self.objective = objective.Objective(pq_dict, startup_penalty,
                                         rebuffer_penalty, smooth_penalty)
  ...
  def observe(self):
    ...
    obs_arr = [
        self.past_chunk_throughputs[-1], self.past_chunk_download_times[-1],
        self.buffer_size, self.total_num_chunks - self.chunk_idx,
        valid_past_action
    ]
    ...
    for i in range(len(obs_arr)):
      if obs_arr[i] > self.obs_high[i]:
        logger.warn('Observation at index ' + str(i) + ' at chunk index ' +
                    str(self.chunk_idx) + ' has value ' + str(obs_arr[i]) +
                    ', which is larger than obs_high ' + str(self.obs_high[i]))
  ...
  def step(self, action):
    ...
    assert action >= 0 and action < len(vid.get_bitrates())
